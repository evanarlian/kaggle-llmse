...
# 1. Load clean train dataset and the original dataset
# 2.
...
if cfg["use_lora"]:
    print("LoRA will automatically freeze layers, overriding...")
...
if cfg["title_trick"]:
    ...
    index = faiss.read_index("input/llmse-paragraph-level-emb-faiss/wiki_notrick.index")
...
train_df = train_df.drop(columns=["context", "source", "science_prob", "is_science"])
...
# TODO might want to force mmap is no memory left by saving the dataset
